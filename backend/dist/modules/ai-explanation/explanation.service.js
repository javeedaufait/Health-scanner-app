...
class ExplanationService {
    async generateFriendlyExplanation(input, language = 'en') {
        if (config_1.config.geminiApiKey || config_1.config.openAiApiKey) {
            try {
    ...
    async generateWithLlm(input, language) {
...
${allergensText ? `Allergens Detected:\n${allergensText}` : ''}`;
    ...
    generateTemplateFallback(input, language) {
        if (language === 'ml') {
