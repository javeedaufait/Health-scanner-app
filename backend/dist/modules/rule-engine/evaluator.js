...
function evaluateFoodForUser(input) {
    const { userProfile, productNutrition, ingredientsList = [], detectedAllergens = [], customRules = shared_1.DEFAULT_HEURISTIC_RULES, } = input;
    ...
    // Check user allergen restrictions against ingredients and detected allergens
    for (const allergen of userAllergens) {
        ...
        if (matchedIngredient) {
            const allergenMeta = shared_1.MASTER_ALLERGENS.find((a) => a.code === allergen);
            ...
            // Allergens impose an immediate severe deduction
            baseScore -= 45;
        }
    }
    ...
    for (const condition of userConditions) {
        ...
        for (const rule of conditionRules) {
            ...
            if (isTriggered) {
                // Prevent duplicate nutrient reasons for same condition (keep highest severity)
                const existingIdx = reasons.findIndex((r) => r.conditionCode === condition && r.nutrient === rule.nutrient);
                if (existingIdx >= 0) {
                    if (rule.deduction > reasons[existingIdx].deduction) {
                        baseScore -= (rule.deduction - reasons[existingIdx].deduction);
                        reasons[existingIdx] = {
